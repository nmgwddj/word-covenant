...
在 `pnpm tauri dev` 的开发构建中，可通过录音控制旁的波形图标选择“开发模拟音频输入”，再使用正常的“开始记录”按钮运行一段本地脚本化会话。它会按采样时钟逐步生成 `synthetic` 转写、匿名说话人标签和本地审计记录，不请求麦克风权限，也不会出网。该能力仅用于验证会话与界面流程，不代表真实麦克风、ASR、声纹或说话人聚类已经完成。
